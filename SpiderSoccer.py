        ...
        if controllersConnected != 2:
            ...
            if players[0].isDead and players[1].isDead:
                ...
                if p2Score + 1 < (boNum + 1) / 2:
                    return run(displayInfo, (p1Score, p2Score + 1), char1, char2, boNum)
                else:
                    print('p2 won')
                    return 5
                ...
                if p1Score + 1 < (boNum + 1) / 2:
                    return run(displayInfo, (p1Score + 1, p2Score), char1, char2, boNum)
                else:
                    print('p1 won')
                    return 4
            # Updates screen
...
class Player():
    def __init__(self, pos, radius, sprite, lengthUnit, mapData, ballColor, num, isZaron = False, ammo = 3000):
        ...
        # ----------------------------------------------
    def knockback(self, xSpe, ySpe):
        self.overrideFrames = 20
        self.lastMovePos = (xSpe, ySpe)
    def reset(self):
        self.pos = self.ogPos
        self.immuneFrames = 20
    def update(self, buttons, aimPos, triggerValue, movePos, downHat, borderInfo):
        ...
        self.immuneFrames = max(0, self.immuneFrames - 1)
        if self.resetFrames == 1:
            self.reset()
            self.resetFrames = 0
        else:
            self.resetFrames = max(0, self.resetFrames - 1)
        if self.resetFrames == 0:
...
class Ball():
    def __init__(self, pos, radius, color = Colors.ORANGE):
        self.radius = radius
    ...
    def updateAngle(self, xSpe, ySpe):
        modifier = 1.5#1
        self.xSpeed -= xSpe * modifier
        self.ySpeed -= ySpe * modifier
        mag = math.hypot(self.xSpeed, self.ySpeed)
        if (mag > self.maxSpeed):
            ang = math.atan2(self.ySpeed, self.xSpeed)
            self.speed = self.maxSpeed
            self.changeAngle(ang)
    def update(self, gameMap, players):
        ...
        for coord in testXCircle:
            try:
                xBad = gameMap.get_at(coord) == Colors.BLACK
                if gameMap.get_at(coord) == (255, 0, 0):
                    return 1
                elif gameMap.get_at(coord) == (0,0,255):
                    return 2
            except:
                xBad = True
        ...
        direc = math.atan2(self.ySpeed, self.xSpeed)
        currentSpeed = math.hypot(self.xSpeed, self.ySpeed)
        minSpeed = 7
        if currentSpeed < minSpeed:
            currentSpeed = 0
        self.speed = min(math.hypot(self.xSpeed, self.ySpeed), self.maxSpeed)
        self.speed *= fric
        self.changeAngle(direc)
        if self.immuneFrames == 0:
            for i in range(len(players)):
                player = players[i]
                distance = math.hypot(self.pos[0] - player.pos[0], self.pos[1] - player.pos[1])
                if player.resetFrames == 0 and distance < self.radius + player.radius:
                    if player.immune:
                            xDisp = player.pos[0] - self.pos[0]
                            yDisp = player.pos[1] - self.pos[1]
                            angle = math.atan2(yDisp, xDisp) + math.pi
                            self.speed = max(2 * self.speed, (1/2) * self.maxSpeed)
...
class Projectile():
    ...
    def fakeUpdate(self, gameMap, players):
        ...
        return value
    def update(self, gameMap, players, gameBall):
        self.immuneFrames -= 1
        self.xNoBounce -= 1
        self.yNoBounce -= 1
        posX = self.pos[0]
        ...
        if self.immuneFrames < 0:
            # gameball collision
            distance = math.hypot(self.pos[0] - gameBall.pos[0], self.pos[1] - gameBall.pos[1])
            if distance < self.radius + gameBall.radius:
                xDisp = gameBall.pos[0] - self.pos[0]
                yDisp = gameBall.pos[1] - self.pos[1]
                currentDir = math.atan2(self.ySpeed, self.xSpeed)
                angle = math.atan2(yDisp, xDisp) + math.pi
                self.changeAngle(angle)
                newDir = math.atan2(self.ySpeed, self.xSpeed) - math.pi
                ...
                if diff < -3.14:
                    diff += 6.28
                diff = abs(diff)
                modifier = (3.14 - diff) / 3.14
                modif2 = 2.2#3
                modifier = pow(modifier, modif2)
                
                gameBall.updateAngle(self.xSpeed*modifier, self.ySpeed*modifier)
                return -1
